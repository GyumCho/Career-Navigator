...
const EditItemForm: React.FC<{category: string, initialFormData: {[key: string]: string}, onSubmit: (item: {[key: string]: string}, index: number) => void, onClose: () => void}> = ({ category, initialFormData, onSubmit, onClose }) => {
  ...
  const handleSubmit = () => {
    if (checkValid()) {
      // TODO: Do something to put in a proper index.
      onSubmit(formData, 0);
    }
  };
